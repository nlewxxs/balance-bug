...
function AddSession() {
    ...
    const url = "http://54.165.136.137:8081/BugInformation/Online?Timeout=inf"
    ...
    const handleSubmit = (event) => {
        ...
        // Make api call
        setLoading(true);
        const url = `http://54.165.136.137:8081/Session/Add?TimeStamp=${dateTime}&BugName=${selectedOption}&SessionName=${sessionName}`;
        fetchData(url)
        .then( response => {
            //console.log(response.data);
            const data = response.data;
