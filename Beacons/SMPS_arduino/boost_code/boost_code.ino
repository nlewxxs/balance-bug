...
float kpv=0.05024,kiv=1.578,kdv=0; // voltage pid.
...
uint8_t buf = 0;      // fpga SPI recv buffer
int fpgaCS = 4;
#define capInput = A4;
...
void sampling(){
  ...
  sensorValue4 = analogRead(A4);  // sample Vcap
  ...
  vCap = sensorValue4 * (1/5.5) * (4.09 / 1023);  // convert v cap to volts
